...
constexpr char kVertexShaderFilename[] = "shaders/wave_object.vert";
constexpr char kFragmentShaderFilename[] = "shaders/wave_object.frag";
constexpr char kUseDepthForOcclusionShaderFlag[] = "USE_DEPTH_FOR_OCCLUSION";
}  // namespace

void ObjWareRenderer::InitializeGlContent(AAssetManager* asset_manager,
  ...
  if (!util::LoadObjFile(obj_file_name, &vertices_, &normals_, &uvs_,
                         &indices_, true)) {
    LOGE("failed to load %s", obj_file_name.c_str());
  }
  LOGI("Load %s #vertices=%d #faces=%d", obj_file_name.c_str(),
       static_cast<int>(vertices_.size() / 3), static_cast<int>(indices_.size() / 3));

  util::CheckGlError("obj_renderer::InitializeGlContent()");

  line_indices_.reserve(indices_.size() * 2);  // Each triangle gives 3 lines (6 indices)

  for (size_t i = 0; i + 2 < indices_.size(); i += 3) {
    GLushort i0 = indices_[i];
    GLushort i1 = indices_[i + 1];
    GLushort i2 = indices_[i + 2];
    // Triangle edges: i0-i1, i1-i2, i2-i0
    line_indices_.push_back(i0); line_indices_.push_back(i1);
    line_indices_.push_back(i1); line_indices_.push_back(i2);
    line_indices_.push_back(i2); line_indices_.push_back(i0);
  }
}

void ObjWareRenderer::setUseDepthForOcclusion(AAssetManager* asset_manager,
...
void ObjWareRenderer::compileAndLoadShaderProgram(AAssetManager* asset_manager) {
...
void ObjWareRenderer::SetMaterialProperty(float ambient, float diffuse,
...
void ObjWareRenderer::Draw(const glm::mat4& projection_mat,
  ...
  glLineWidth(2);
  glDrawElements(GL_LINES, line_indices_.size(), GL_UNSIGNED_SHORT, line_indices_.data());
