...
BleMessageHandler ble_message_handler_ = NULL;
void SetBleMessageHandler(BleMessageHandler fcn) {
  ble_message_handler_ = fcn;
}
...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "BOTMOKUKU_DEMO";
...
static uint8_t raw_adv_data[] = {
    /* Flags */
    0x02, ESP_BLE_AD_TYPE_FLAG, 0x06,  // Length 2, Data Type ESP_BLE_AD_TYPE_FLAG, Data 1 (LE
    ...
    0x03, ESP_BLE_AD_TYPE_16SRV_CMPL, 0xAB,
    0xCD  // Length 3, Data Type ESP_BLE_AD_TYPE_16SRV_CMPL, Data 3 (UUID)
};
...
static uint8_t raw_scan_rsp_data[] = {
    ...
    'U',  '_',
    'D',  'E',
    'M',  'O'  // Length 15, Data Type ESP_BLE_AD_TYPE_NAME_CMPL, Data (ESP_GATTS_DEMO)
};
#else
...
static struct gatts_profile_inst gl_profile_tab[PROFILE_NUM] = {
    ...
    [PROFILE_B_APP_ID] =
        {
            .gatts_cb = gatts_profile_b_event_handler, /* This demo does not implement, similar as
                                                          profile A */
            .gatts_if = ESP_GATT_IF_NONE, /* Not get the gatt_if, so initial is ESP_GATT_IF_NONE */
        },
};
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param) {
  switch (event) {
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
      ESP_LOGI(GATTS_TAG,
               "Connection params update, status %d, conn_int %d, latency %d, timeout %d",
    ...
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
      ESP_LOGI(GATTS_TAG, "Packet length update, status %d, rx %d, tx %d",
               param->pkt_data_length_cmpl.status, param->pkt_data_length_cmpl.params.rx_len,
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
  ...
  switch (event) {
    ...
    case ESP_GATTS_READ_EVT: {
      ESP_LOGI(GATTS_TAG,
               "Characteristic read request: conn_id=%d, trans_id=%" PRIu32
               ", handle=%d, is_long=%d, offset=%d, need_rsp=%d",
               param->read.conn_id, param->read.trans_id, param->read.handle, param->read.is_long,
               param->read.offset, param->read.need_rsp);
      ...
      if (param->read.handle == gl_profile_tab[PROFILE_A_APP_ID].char_handle) {
        ...
        if (param->read.is_long && offset > CONFIG_EXAMPLE_CHAR_READ_DATA_LEN) {
          ESP_LOGW(GATTS_TAG, "Read offset (%d) out of range (0-%d)", offset,
                   CONFIG_EXAMPLE_CHAR_READ_DATA_LEN);
        ...
        uint16_t send_len = (CONFIG_EXAMPLE_CHAR_READ_DATA_LEN - offset > mtu_size)
                                ? mtu_size
                                : (CONFIG_EXAMPLE_CHAR_READ_DATA_LEN - offset);
    ...
    case ESP_GATTS_WRITE_EVT: {
      ESP_LOGI(GATTS_TAG, "A Characteristic write, conn_id %d, trans_id %" PRIu32 ", handle %d",
               param->write.conn_id, param->write.trans_id, param->write.handle);
      if (!param->write.is_prep) {
        // ESP_LOGI(GATTS_TAG, "A value len %d, value ", param->write.len);
        // ESP_LOG_BUFFER_HEX(GATTS_TAG, param->write.value, param->write.len);
        if (ble_message_handler_) {
          ble_message_handler_(param->write.len, param->write.value);
        }
    ...
    case ESP_GATTS_CREATE_EVT:
      ESP_LOGI(GATTS_TAG, "Service create, status %d, service_handle %d", param->create.status,
    ...
    case ESP_GATTS_START_EVT:
      ESP_LOGI(GATTS_TAG, "Service start, status %d, service_handle %d", param->start.status,
    ...
    case ESP_GATTS_CONNECT_EVT: {
      ...
      ESP_LOGI(GATTS_TAG, "A Connected, conn_id %u, remote " ESP_BD_ADDR_STR "",
    ...
    case ESP_GATTS_DISCONNECT_EVT:
      ESP_LOGI(GATTS_TAG, "A Disconnected, remote " ESP_BD_ADDR_STR ", reason 0x%02x",
               ESP_BD_ADDR_HEX(param->disconnect.remote_bda), param->disconnect.reason);
      esp_ble_gap_start_advertising(&adv_params);
      local_mtu = 23;  // Reset MTU for a single connection
      break;
    case ESP_GATTS_CONF_EVT:
      ESP_LOGI(GATTS_TAG, "A Confirm receive, status %d, attr_handle %d", param->conf.status,
               param->conf.handle);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
  ...
  switch (event) {
    ...
    case ESP_GATTS_CONNECT_EVT:
      ...
      break;
    case ESP_GATTS_CONF_EVT:
      ESP_LOGI(GATTS_TAG, "Confirm receive, status %d, attr_handle %d", param->conf.status,
               param->conf.handle);
      if (param->conf.status != ESP_GATT_OK) {
        ESP_LOG_BUFFER_HEX(GATTS_TAG, param->conf.value, param->conf.len);
      }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
  ...
  if (event == ESP_GATTS_REG_EVT) {
    if (param->reg.status == ESP_GATT_OK) {
      ...
      ESP_LOGI(GATTS_TAG, "Reg app failed, app_id %04x, status %d", param->reg.app_id,
               param->reg.status);
      return;
    }
  }
